...
java -jar ../build/libs/great-versioning-tool-1.0.jar detach > message.out
if [[ $? -ne 30 ]]; then
    cd -
...
java -jar ../build/libs/great-versioning-tool-1.0.jar detach c.txt> message.out
if [[ $? -ne 0 ]]; then
    cd -
...
java -jar ../build/libs/great-versioning-tool-1.0.jar detach a.txt -m "a file is not needed"> message.out
if [[ $? -ne 0 ]]; then
    cd -
...
if [[ $(java -jar ../build/libs/great-versioning-tool-1.0.jar history -last 1) = "4: Detached file: a.txt" ]]; then
...
java -jar ../build/libs/great-versioning-tool-1.0.jar version > message.out
